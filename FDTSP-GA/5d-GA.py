...
def main(name_file, instances_path, soluciones_folder):
    ...
    speed_truck = 35                                                           # mph
    endurance1 = int(endurance)/60 ### AGREGAR                                                      # hour
    endurance2 = int(endurance)/60 ### AGREGAR
    ...
    for seed in range(1, nrSeeds + 1):
        ...
        for it in range(0,iter_proposed_model):
            ...
            c = 300 # iteraciones bisecting_kmeans ### AGREGAR                          
            ...
            for i, idx in enumerate(noLaunched_indices_d1):
                list_dr1_new[idx+count_total] = []
                idx +=1
            for i, idx in enumerate(noLaunched_indices_d2):
                list_dr2_new[idx+count_total] = []
                idx +=1
            for i, idx in enumerate(noLaunched_indices_d3):
                list_dr3_new[idx+count_total] = []
                idx +=1
            for i, idx in enumerate(noLaunched_indices_d4):
                list_dr4_new[idx+count_total] = []
                idx +=1
            for i, idx in enumerate(noLaunched_indices_d5):
                list_dr5_new[idx+count_total] = []
                idx +=1
            ###
            ...
            # pd.DataFrame(trucks_new).to_csv("trucks_new_2drones.csv", index=False)      
            ...
            def load_csv():                                                           
                ...
                return x, y
            def initialization(x,y, indice): ### AGREGAR
                ini_solution=[]    # path of customer chromosome
                for i in range(chromosome):
                    t = random.sample(range(0, numberofcustomer), numberofcustomer)
                    t.remove(indice) ### AGREGAR
                    t.insert(0,indice) ### AGREGAR
                    ini_solution.append(t)  
                return ini_solution
            def distance(solution):   #calculate distance
                totaldis = []
                ...
                for x in range(mutation_miltiplier):
                    for i in range(chromosome):
                        ...
                        if alpha < mutation_rate:
                            draw1 = random.randint(1, numberofcustomer - 1) ### AGREGAR
                            draw2 = random.randint(1, numberofcustomer - 1) ### AGREGAR
                            while (draw2==draw1):
                                draw2 = random.randint(1, numberofcustomer - 1) ### AGREGAR
                            temp_7 = []
                            for j in range(numberofcustomer):
            ...
            # print('t- Nodos asignados a camion: ', trucks_new.tolist())
            # r_ordenada = [trucks_new[i] for i in road_map]
            ...
            for i in range(len(road_map)):
                if list_dr2_new[road_map[i]]!=[]:
                    ...
                    for j in vuelos: ### AGREGAR
                        if j[0] == list_dr2_new[road_map[i]][0] and j[1] == list_dr2_new[road_map[i]][1]:
                            vuelos[j].append(list_trucks_new[road_map[i]])
                            break
                else:
                    d2_tr.append(0)
            ...
            for i in range(len(road_map)):
                if list_dr3_new[road_map[i]]!=[]:
                    ...
                    for j in vuelos: ### AGREGAR
                        if j[0] == list_dr3_new[road_map[i]][0] and j[1] == list_dr3_new[road_map[i]][1]:
                            vuelos[j].append(list_trucks_new[road_map[i]])
                            break
                else:
                    d3_tr.append(0)
            d4_tr = []
            for i in range(len(road_map)):
                if list_dr4_new[road_map[i]]!=[]:
                    tempo4 = math.sqrt((list_trucks_new[road_map[i]][0]-list_dr4_new[road_map[i]][0])**2+(list_trucks_new[road_map[i]][1]-list_dr4_new[road_map[i]][1])**2)
                    d4_tr.append(tempo4)

                    for j in vuelos: ### AGREGAR
                        if j[0] == list_dr4_new[road_map[i]][0] and j[1] == list_dr4_new[road_map[i]][1]:
                            vuelos[j].append(list_trucks_new[road_map[i]])
                            break
                else:
                    d4_tr.append(0)
            d5_tr = []
            for i in range(len(road_map)):
                if list_dr5_new[road_map[i]]!=[]:
                    tempo5 = math.sqrt((list_trucks_new[road_map[i]][0]-list_dr5_new[road_map[i]][0])**2+(list_trucks_new[road_map[i]][1]-list_dr5_new[road_map[i]][1])**2)
                    d5_tr.append(tempo5)

                    for j in vuelos: ### AGREGAR
                        if j[0] == list_dr5_new[road_map[i]][0] and j[1] == list_dr5_new[road_map[i]][1]:
                            vuelos[j].append(list_trucks_new[road_map[i]])
                            break
                else:
                    d5_tr.append(0)
            ...
            for i in range(len(road_map)-1):
                if list_dr4_new[road_map[i]]!=[]:
                    tempo14 = math.sqrt((list_dr4_new[road_map[i]][0]-list_trucks_new[road_map[i+1]][0])**2+(list_dr4_new[road_map[i]][1]-list_trucks_new[road_map[i+1]][1])**2)
            ...
            for i in range(len(road_map)-1,len(road_map)):
                if list_dr4_new[road_map[i]]!=[]:
                    tempo104 = math.sqrt((list_dr4_new[road_map[i]][0]-list_trucks_new[road_map[0]][0])**2+(list_dr4_new[road_map[i]][1]-list_trucks_new[road_map[0]][1])**2)
            ...
            for i in range(len(road_map)-1):
                if list_dr5_new[road_map[i]]!=[]:
                    tempo15 = math.sqrt((list_dr5_new[road_map[i]][0]-list_trucks_new[road_map[i+1]][0])**2+(list_dr5_new[road_map[i]][1]-list_trucks_new[road_map[i+1]][1])**2)
            ...
            for i in range(len(road_map)-1,len(road_map)):
                if list_dr5_new[road_map[i]]!=[]:
                    tempo105 = math.sqrt((list_dr5_new[road_map[i]][0]-list_trucks_new[road_map[0]][0])**2+(list_dr5_new[road_map[i]][1]-list_trucks_new[road_map[0]][1])**2)
            ...
            for k in range(len(d2_tr)):
                if d2_travel[k] > endurance2*delta*speed_drone and 2*d2_tr[k] < (endurance2-(0.5/60))*speed_drone:
                    ...
                    d2_ntr_new[k] = 'COMEBACK'  #RETURNED NODES
            ...
            for k in range(len(d3_tr)):
                if d3_travel[k] > endurance2*delta*speed_drone and 2*d3_tr[k] < (endurance2-(0.5/60))*speed_drone:
                    ...
                    d3_ntr_new[k] = 'COMEBACK'  #RETURNED NODES
                ...
                if list_dr1_new[road_map[i]]!=[] and d1_tr_new[i] == 'COMEBACK':
                    ...
                    for j in vuelos: 
                        if j[0] == list_dr1_new[road_map[i]][0] and j[1] == list_dr1_new[road_map[i]][1]:
                            ...
                            break
            for i in range(len(road_map)):
                if list_dr2_new[road_map[i]]!=[] and d2_tr_new[i] == 'COMEBACK':
                    # print('i: ', i, 'list_dr1_new[road_map[i]]: ', list_dr1_new[road_map[i]])
                    for j in vuelos:
                        if j[0] == list_dr2_new[road_map[i]][0] and j[1] == list_dr2_new[road_map[i]][1]:
                            # print('vuelo antes: ', vuelos[j])
                            ...
                            break
            for i in range(len(road_map)):
                if list_dr3_new[road_map[i]]!=[] and d3_tr_new[i] == 'COMEBACK':
                    # print('i: ', i, 'list_dr1_new[road_map[i]]: ', list_dr1_new[road_map[i]])
                    for j in vuelos: 
                        if j[0] == list_dr3_new[road_map[i]][0] and j[1] == list_dr3_new[road_map[i]][1]:
                            # print('vuelo antes: ', vuelos[j])
                            ...
                            break
            for i in range(len(road_map)):
                if list_dr4_new[road_map[i]]!=[] and d4_tr_new[i] == 'COMEBACK':
                    # print('i: ', i, 'list_dr1_new[road_map[i]]: ', list_dr1_new[road_map[i]])
                    for j in vuelos: 
                        if j[0] == list_dr4_new[road_map[i]][0] and j[1] == list_dr4_new[road_map[i]][1]:
                            # print('vuelo antes: ', vuelos[j])
                            ...
                            break
            for i in range(len(road_map)):
                if list_dr5_new[road_map[i]]!=[] and d5_tr_new[i] == 'COMEBACK':
                    # print('i: ', i, 'list_dr1_new[road_map[i]]: ', list_dr1_new[road_map[i]])
                    for j in vuelos: 
                        if j[0] == list_dr5_new[road_map[i]][0] and j[1] == list_dr5_new[road_map[i]][1]:
                            # print('vuelo antes: ', vuelos[j])
            ...
            within_d5 = [sum(v) for v in zip(travel_d4_tr,setup_d5,service_d5)]
            ...
            if d1_tr_new.count('NO LAUNCHED 2') == 0 and d2_tr_new.count('NO LAUNCHED 2') ==0 and d3_tr_new.count('NO LAUNCHED 2') ==0 and d4_tr_new.count('NO LAUNCHED 2') ==0 and d5_tr_new.count('NO LAUNCHED 2') ==0:
                ...
                for i in indices_2:
                    nueva_list_trucks_new.append(list_dr2_new[road_map[i]])
                    indice = nueva_list_trucks_new.index(list_dr2_new[road_map[i]])
                    nueva_road_map.append(indice)
                    list_dr2_new[road_map[i]] = []
                    nueva_waiting_time_dr1.append(0)
                    nueva_waiting_time_dr2.append(0)
                    nueva_waiting_time_dr3.append(0)
                    nueva_waiting_time_dr4.append(0)
                    ...
                    nueva_time_d5.append(0)
                for i in indices_3:
                    nueva_list_trucks_new.append(list_dr3_new[road_map[i]])
                    indice = nueva_list_trucks_new.index(list_dr3_new[road_map[i]])
                    nueva_road_map.append(indice)
                    list_dr3_new[road_map[i]] = []
                    nueva_waiting_time_dr1.append(0)
                    nueva_waiting_time_dr2.append(0)
                    nueva_waiting_time_dr3.append(0)
                    nueva_waiting_time_dr4.append(0)
                    ...
                    nueva_time_d5.append(0)
                for i in indices_4:
                    nueva_list_trucks_new.append(list_dr4_new[road_map[i]])
                    indice = nueva_list_trucks_new.index(list_dr4_new[road_map[i]])
                    nueva_road_map.append(indice)
                    list_dr4_new[road_map[i]] = []
                    nueva_waiting_time_dr1.append(0)
                    nueva_waiting_time_dr2.append(0)
                    nueva_waiting_time_dr3.append(0)
                    nueva_waiting_time_dr4.append(0)
                    ...
                    nueva_time_d5.append(0)
                for i in indices_5:
                    nueva_list_trucks_new.append(list_dr5_new[road_map[i]])
                    indice = nueva_list_trucks_new.index(list_dr5_new[road_map[i]])
                    nueva_road_map.append(indice)
                    list_dr5_new[road_map[i]] = []
                    nueva_waiting_time_dr1.append(0)
                    nueva_waiting_time_dr2.append(0)
                    nueva_waiting_time_dr3.append(0)
                    nueva_waiting_time_dr4.append(0)
