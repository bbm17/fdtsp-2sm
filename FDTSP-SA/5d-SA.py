...
def main(name_file, instances_path, soluciones_folder):
    ...
    class SimAnneal(object):
        def __init__(self, coords, tpo_inicio, tpo_lim, T=-1, alpha=-1, stopping_T=-1, stopping_iter=-1, idx_deposito = 0): ### AGREGAR
            self.coords = coords
            self.N = len(coords)
            self.T = math.sqrt(self.N) if T == -1 else T
            self.T_save = self.T 
            self.alpha = 0.995 if alpha == -1 else alpha
            self.stopping_temperature = 1e-10000 if stopping_T == -1 else stopping_T ### AGREGAR, se camibio la temp
        ...
        def anneal(self):
            ...
            while self.iteration <= self.stopping_iter and self.T >= self.stopping_temperature: ### AGREGAR, se camion el signo
        ...
        def get_routes(self):
            # print('best sol SA: ', self.best_solution)
    ...
    d_deliveries = []
    s_levels = []
    contador_factibilidad = 0 ### AGREGAR
    for seed in range(1, nrSeeds + 1):
        ...
        for it in range(0,iter_proposed_model):
            ...
            c = 300 # iteraciones bisecting_kmeans ### AGREGAR, se cambiaron las it
            ...
            #SEPARATING NODES OF EACH DRONE
            list_dr1 = []
            list_dr2 = []
            list_dr3 = []
            list_dr4 = []
            ...
            for i in range(len(list_dr4)):
                if list_dr4[i]!=[]:
                    ...
                    d4_tr_test.append(0)
            d5_tr_test = []
            for i in range(len(list_dr5)):
                if list_dr5[i]!=[]:
                    dr5_test = math.sqrt((list_trucks[i][0]-list_dr5[i][0])**2+(list_trucks[i][1]-list_dr5[i][1])**2)
                    d5_tr_test.append(dr5_test)
                else:
                    d5_tr_test.append(0)
            #TESTING
            ...
            d5_tr_test_note = [random.randrange(1, 1000, 1) for i in range(len(d5_tr_test))]
            ### AGREGAR , todos los delta
            ...
            for k in range(len(d5_tr_test)):
                if d5_tr_test[k] > (endurance2*delta*speed_drone)/2:
                    d5_tr_test_note[k] = 'NO LAUNCHED'
                    # print('d5- El nodo no fue lanzado: ',list_dr5[k])
                else:
                    d5_tr_test_note[k] = d5_tr_test[k]
            # REPLACED NODES
            ...
            noLaunched_indices_d5 = [index for index, element in enumerate(d5_tr_test_note) if element == "NO LAUNCHED"]
            ...
            count5 = len(noLaunched_indices_d5)

            list_dr1_new = list_dr1.copy()
            list_dr2_new = list_dr2.copy()
            list_dr3_new = list_dr3.copy()
            list_dr4_new = list_dr4.copy()
            list_dr5_new = list_dr5.copy()

            #### En esta etapa los nodos de drones son insertados en la ruta del camion ### AGREGAR
            k = 0
            ...
            for i, idx in enumerate(noLaunched_indices_d4):
                ...
                k +=1
            k = 0
            for i, idx in enumerate(noLaunched_indices_d5):
                list_trucks.insert(i, list_dr5_new[idx + k + count1 + count2 + count3 + count4])
                list_dr1_new.insert(i, [])
                list_dr2_new.insert(i, [])
                list_dr3_new.insert(i, [])
                list_dr4_new.insert(i, [])
            ...
            count_total = count1 + count2 + count3 + count4 + count5 ### AGREGAR
            for i, idx in enumerate(noLaunched_indices_d1):
            ...
            for i, idx in enumerate(noLaunched_indices_d4):
                ...
                idx +=1
            for i, idx in enumerate(noLaunched_indices_d5):
                list_dr5_new[idx+count_total] = [] ### AGREGAR
                idx +=1
            ...
            # data = pd.read_csv(soluciones_folder + '/' + "trucks_new"+ '_' + name_file[:4] + '_' + str(n) + '_' + str(endurance) + '_' + str(speed_drone) + '_' + str(dr) + '_' + str(seed) +".csv")                                              # call the truck nodes
        ...
        ############################## SIMULATED ANNEALING BEGINS #############################################
            ...
            if len(coords1) <= 2:
                ...
                sa = SimAnneal(coords1, inicio, tiempoLimite, stopping_iter=100000000, idx_deposito = indice_deposito) #5000  ### AGREGAR
                sa.anneal()
                #sa.visualize_routes()
        ...
        ############################## COMBINATION BETWEEN TRUCK AND DRONES BEGIN ###########################
            ...
            # print('d5- Nodos asigandos a dron: ', list_dr5_new)
            ...
            for i in range(len(road_map_sa_truck_nodes)):
                if list_dr1_new[road_map_sa_truck_nodes[i]]!=[]:
                    # print('d1-> i: ', i, 'road_map_sa_truck_nodes[i]: ', road_map_sa_truck_nodes[i], 'list_trucks_new[road_map_sa_truck_nodes[i]]: ',list_trucks_new[road_map_sa_truck_nodes[i]], 'list_dr1_new[road_map_sa_truck_nodes[i]]: ', list_dr1_new[road_map_sa_truck_nodes[i]])
            ...
            for i in range(len(road_map_sa_truck_nodes)):
                if list_dr2_new[road_map_sa_truck_nodes[i]]!=[]:
                    # print('d2-> i: ', i, 'road_map_sa_truck_nodes[i]: ', road_map_sa_truck_nodes[i], 'list_trucks_new[road_map_sa_truck_nodes[i]]: ',list_trucks_new[road_map_sa_truck_nodes[i]], 'list_dr1_new[road_map_sa_truck_nodes[i]]: ', list_dr2_new[road_map_sa_truck_nodes[i]])
            ...
            for i in range(len(road_map_sa_truck_nodes)):
                if list_dr3_new[road_map_sa_truck_nodes[i]]!=[]:
                    # print('d3-> i: ', i, 'road_map_sa_truck_nodes[i]: ', road_map_sa_truck_nodes[i], 'list_trucks_new[road_map_sa_truck_nodes[i]]: ',list_trucks_new[road_map_sa_truck_nodes[i]], 'list_dr1_new[road_map_sa_truck_nodes[i]]: ', list_dr3_new[road_map_sa_truck_nodes[i]])
            ...
            for i in range(len(road_map_sa_truck_nodes)):
                if list_dr4_new[road_map_sa_truck_nodes[i]]!=[]:
                    # print('d4-> i: ', i, 'road_map_sa_truck_nodes[i]: ', road_map_sa_truck_nodes[i], 'list_trucks_new[road_map_sa_truck_nodes[i]]: ',list_trucks_new[road_map_sa_truck_nodes[i]], 'list_dr1_new[road_map_sa_truck_nodes[i]]: ', list_dr4_new[road_map_sa_truck_nodes[i]])
            ...
            d5_tr = []
            for i in range(len(road_map_sa_truck_nodes)):
                if list_dr5_new[road_map_sa_truck_nodes[i]]!=[]:
                    # print('d5-> i: ', i, 'road_map_sa_truck_nodes[i]: ', road_map_sa_truck_nodes[i], 'list_trucks_new[road_map_sa_truck_nodes[i]]: ',list_trucks_new[road_map_sa_truck_nodes[i]], 'list_dr1_new[road_map_sa_truck_nodes[i]]: ', list_dr5_new[road_map_sa_truck_nodes[i]])
                    tempo5 = math.sqrt((list_trucks_new[road_map_sa_truck_nodes[i]][0]-list_dr5_new[road_map_sa_truck_nodes[i]][0])**2+\
                                    (list_trucks_new[road_map_sa_truck_nodes[i]][1]-list_dr5_new[road_map_sa_truck_nodes[i]][1])**2)
                    d5_tr.append(tempo5)
                    
                    for j in vuelos: ### AGREGAR
                        if j[0] == list_dr5_new[road_map_sa_truck_nodes[i]][0] and j[1] == list_dr5_new[road_map_sa_truck_nodes[i]][1]:
                            vuelos[j].append(list_trucks_new[road_map_sa_truck_nodes[i]])
                            break
            ...
            for i in range(len(road_map_sa_truck_nodes)-1):
                if list_dr1_new[road_map_sa_truck_nodes[i]]!=[]:
                    # if round(list_dr1_new[road_map_sa_truck_nodes[i]][0], 3) == 38.741 and round(list_dr1_new[road_map_sa_truck_nodes[i]][1], 3) == 11.563:
                        # print('d1.1-> i: ', i, 'road_map_sa_truck_nodes[i]: ', road_map_sa_truck_nodes[i], 'list_dr1_new[road_map_sa_truck_nodes[i]]: ', list_dr1_new[road_map_sa_truck_nodes[i]], 'list_trucks_new[road_map_sa_truck_nodes[i+1]]: ',list_trucks_new[road_map_sa_truck_nodes[i+1]])
            ...
            for i in range(len(road_map_sa_truck_nodes)-1,len(road_map_sa_truck_nodes)):
                if list_dr1_new[road_map_sa_truck_nodes[i]]!=[]:
                    # print('d1.2-> i: ', i, 'road_map_sa_truck_nodes[i]: ', road_map_sa_truck_nodes[i], 'list_dr1_new[road_map_sa_truck_nodes[i]]: ', list_dr1_new[road_map_sa_truck_nodes[i]], 'list_trucks_new[road_map_sa_truck_nodes[0]]: ',list_trucks_new[road_map_sa_truck_nodes[0]])
            ...
            for i in range(len(road_map_sa_truck_nodes)-1):
                if list_dr5_new[road_map_sa_truck_nodes[i]]!=[]:
                    tempo15 = math.sqrt((list_dr5_new[road_map_sa_truck_nodes[i]][0]-list_trucks_new[road_map_sa_truck_nodes[i+1]][0])**2+(list_dr5_new[road_map_sa_truck_nodes[i]][1]-list_trucks_new[road_map_sa_truck_nodes[i+1]][1])**2)
                    d5_ntr.append(tempo15) 
            ...
            for i in range(len(road_map_sa_truck_nodes)-1,len(road_map_sa_truck_nodes)):
                if list_dr5_new[road_map_sa_truck_nodes[i]]!=[]:
                    tempo105 = math.sqrt((list_dr5_new[road_map_sa_truck_nodes[i]][0]-list_trucks_new[road_map_sa_truck_nodes[0]][0])**2+(list_dr5_new[road_map_sa_truck_nodes[i]][1]-list_trucks_new[road_map_sa_truck_nodes[0]][1])**2)
                    d5_ntr.append(tempo105)
            ...
            d5_travel = list(map(add,d5_tr,d5_ntr))
            # print('di_travel = ', d1_travel, d2_travel, d3_travel, d4_travel, d5_travel) # Todos las distancias de viaje
            
            ## El problema es que pueden haber tiempos de espera del dron que exceden el lim de batería, por lo tanto puede ser infactible.
            ...
            d5_tr_new = [random.randrange(1, 1000, 1) for i in range(len(d5_tr))]
            d5_ntr_new = [random.randrange(1, 1000, 1) for i in range(len(d5_ntr))]
            for k in range(len(d5_tr)):
                if d5_travel[k] > endurance2*delta*speed_drone and 2*d5_tr[k] < (endurance2-(0.5/60))*speed_drone:
                    d5_tr_new[k] = 'COMEBACK'
                    d5_ntr_new[k] = 'COMEBACK' #RETURNED NODES 5TH DRONE
                elif d5_travel[k] > endurance2*delta*speed_drone and 2*d5_tr[k] > (endurance2-(0.5/60))*speed_drone:
                    d5_tr_new[k] = 'NO LAUNCHED 2'
                    d5_ntr_new[k] = 'NO LAUNCHED 2' # REPLACED NODES 5TH DRONE (IF)
                else:
                    d5_tr_new[k] = d5_tr[k]
                    d5_ntr_new[k] =  d5_ntr[k]

            ### AGREGAR, con esto se modifican los vuelos guardados
            for i in range(len(road_map_sa_truck_nodes)):
                ...
                if list_dr4_new[road_map_sa_truck_nodes[i]]!=[] and d4_tr_new[i] == 'COMEBACK':
                    ...
                    for j in vuelos: 
                        if j[0] == list_dr4_new[road_map_sa_truck_nodes[i]][0] and j[1] == list_dr4_new[road_map_sa_truck_nodes[i]][1]:
                            ...
                            break
            for i in range(len(road_map_sa_truck_nodes)):
                if list_dr5_new[road_map_sa_truck_nodes[i]]!=[] and d5_tr_new[i] == 'COMEBACK':
                    # print('i: ', i, 'list_dr1_new[road_map_sa_truck_nodes[i]]: ', list_dr1_new[road_map_sa_truck_nodes[i]])
                    for j in vuelos: 
                        if j[0] == list_dr5_new[road_map_sa_truck_nodes[i]][0] and j[1] == list_dr5_new[road_map_sa_truck_nodes[i]][1]:
                            # print('vuelo antes: ', vuelos[j])
                            ...
                            # print('vuelo dsp: ', vuelos[j])
                            break
            # print('--> VUELOS: ', vuelos) #Ultima modificación de los vuelos
            ...
            d5_tr_new_temp = [random.randrange(1, 1000, 1) for i in range(len(d5_tr_new))]
            d5_ntr_new_temp = [random.randrange(1, 1000, 1) for i in range(len(d5_ntr_new))]
            for k in range(len(d5_tr_new)):
                if d5_tr_new[k] =='COMEBACK' or d5_tr_new[k]=='NO LAUNCHED 2':
                    d5_tr_new_temp[k] = 0
                    d5_ntr_new_temp[k] = 0 
                else:
                    d5_tr_new_temp[k] = d5_tr_new[k]
                    d5_ntr_new_temp[k] = d5_ntr_new[k]
            #TRAVEL TIME 
            ...
            travel_d5_tr = [60*within_distance5/speed_drone for within_distance5 in d5_tr_new_temp]
            # print('tiempos de viaje: ', travel_d1_tr, travel_d2_tr, travel_d3_tr, travel_d4_tr, travel_d5_tr) # Tiempos de viaje en min
            #SETUP TIME
            ...
            for k in range(len(d4_tr_new)):
                if d4_tr_new[k] =='NO LAUNCHED 2' or d4_tr_new[k]== 0:
                    ...
                    setup_d4[k] = 1 
            setup_d5 = [random.randrange(1, 1000, 1) for i in range(len(d5_tr_new))]
            for k in range(len(d5_tr_new)):
                if d5_tr_new[k] =='NO LAUNCHED 2' or d5_tr_new[k]== 0:
                    setup_d5[k] = 0
                else:
                    setup_d5[k] = 1 #
            #SERVICE TIME
            ...
            service_d5 = [random.randrange(1, 1000, 1) for i in range(len(d5_tr_new))]
            ...
            within_d5 = [sum(v) for v in zip(travel_d5_tr,setup_d5,service_d5)] ### AGREGAR, habia un 4 en vez de 5
            ...
            travel_d5_ntr = [60*within_distance5/speed_drone for within_distance5 in d5_ntr_new_temp]
            ...
            time_d5  = [sum(t) for t in zip(within_d5,travel_d5_ntr)]
            time_drone = [max(time_d1[i],time_d2[i],time_d3[i],time_d4[i],time_d5[i]) for i in range(len(time_d1))]
            ...
            waiting_distance_dr5 = [random.randrange(1, 1000, 1) for i in range(len(d5_tr_new))]
            for k in range(len(d5_tr_new)):
                if d5_tr_new[k] == 'COMEBACK':
                    waiting_distance_dr5[k] = 2*d5_tr[k]
                else:
                    waiting_distance_dr5[k] = 0
            waiting_time_dr5 = [60*within_distance5/speed_drone for within_distance5 in waiting_distance_dr5]
            
            waiting_drone = [max(waiting_time_dr1[i],waiting_time_dr2[i],waiting_time_dr3[i],waiting_time_dr4[i],waiting_time_dr5[i]) for i in range(len(waiting_time_dr1))]
            
            ### AGREGAR
            if d1_tr_new.count('NO LAUNCHED 2') == 0 and d2_tr_new.count('NO LAUNCHED 2') ==0 and d3_tr_new.count('NO LAUNCHED 2') ==0 and d4_tr_new.count('NO LAUNCHED 2') ==0 and d5_tr_new.count('NO LAUNCHED 2') ==0:
                ...
                for i in indices_5: ### AGREGAR
                    nueva_list_trucks_new.append(list_dr5_new[road_map_sa_truck_nodes[i]])
                    indice = nueva_list_trucks_new.index(list_dr5_new[road_map_sa_truck_nodes[i]])
                    nueva_road_map_sa_truck_nodes.append(indice)
                    list_dr5_new[road_map_sa_truck_nodes[i]] = []
                    nueva_waiting_time_dr1.append(0)
                    nueva_waiting_time_dr2.append(0)
                    nueva_waiting_time_dr3.append(0)
                    nueva_waiting_time_dr4.append(0)
                ...
                waiting_drone = [max(nueva_waiting_time_dr1[i],nueva_waiting_time_dr2[i],nueva_waiting_time_dr3[i],nueva_waiting_time_dr4[i],nueva_waiting_time_dr5[i]) for i in range(len(nueva_waiting_time_dr1))] #nueva_waiting_drone
                ...
                ### AGREGAR, ultima modificación de el truckroute
                road_map_sa_truck_nodes = nueva_road_map_sa_truck_nodes.copy()
                list_trucks_new = copy.deepcopy(nueva_list_trucks_new)
                
                ### AGREGAR
                del (indices_1,indices_2,indices_3,indices_4,indices_5,
                nueva_list_trucks_new,nueva_road_map_sa_truck_nodes,
                nueva_waiting_time_dr1,nueva_waiting_time_dr2,nueva_waiting_time_dr3,nueva_waiting_time_dr4,nueva_waiting_time_dr5, 
                nueva_time_d1,nueva_time_d2,nueva_time_d3,nueva_time_d4,nueva_time_d5)
